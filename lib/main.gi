...
Labels_SU := function( length, total)
    ...
    if length = 1 then 
        ...
        for i in [0..total] do 
            for t in Labels_SU( length-1,total-i ) do
                Append(t,[i]);
                Append(lista,[t]);
            od;
        od;
...
####
InstallGlobalFunction( Labels_A, function( length, total )
    local lista, i;
...
Numero_orbitas := function( n, k )
    ...
    return [
        Size( Labels_A_ext( n, k ) ),
        Size( orb ),
        Size( orb )/Size( Labels_A_ext( n, k ) )
    ];
end;
...
InstallGlobalFunction( Tmatrix, function( R, k, labels)
...
#########
InstallGlobalFunction( ModularData_TipoA, function( n, k )
    ...
    return rec( Smatrix := S, Tmatrix := T, labels := Labels f.Smatrix);
end; );
...
#####
InstallGlobalFunction( IsModularData, function(S,T)
    local v1, v2, v3, D;
...
InstallGlobalFunction( Display_labels, function(n, k)
    local labels, x, T, R;
    labels := Labels_A(n,k);
    R := RootSystem( SimpleLieAlgebra( "A", n, Rationals ) );;
    Print("\n");
...
InstallGlobalFunction( FusionAlgebra,function(n,k)
...
InstallGlobalFunction( Display_Info_Zesting, function(n,k)
    ...
    for a in A do
        for b in B do 
            if RemInt(a*((n+1)/l)-tempforal(a)*e^a-2*b,n+1)=0 then
                 Append(Zestings,[[a,b]]);
                 Print([a,b]);
                 Print("\n");
            fi;    
...
InstallGlobalFunction( Zesting_Modular_Data, function( n, k, pair, c )
...
InstallGlobalFunction( Zesting_FusionAlgebra, function(n,k,par)
    local T,x,y,d,Alg,base,Base,N;
...
####
InstallGlobalFunction( FS_indicator, function(N,k,n)
