...
InstallGlobalFunction( CentralCharge, function(R, k)
    local c, n, x;
...
InstallGlobalFunction( DagaMatrix, function(U)
...
InstallGlobalFunction( IsUnitaryMatrix, function( U )
    local v, n;
    n := Size( U );
    v := false;
...
InstallGlobalFunction( Gauss_Sum, function(S, T, signo)
    local k, s, x;
    s := Size( S );
    k := 0;
...
InstallGlobalFunction( Test_modular, function(n,k)
    local MD, test;
